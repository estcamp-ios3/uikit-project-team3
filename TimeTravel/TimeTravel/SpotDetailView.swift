...
class SpotDetailView: UIViewController {
    ...
    @objc private func storyButtonTapped() {
        ...
        storyVC.view.backgroundColor = .systemTeal // 배경색으로 구분
        storyVC.title = "이야기 상세" // 네비게이션 바 제목
        
        let label = UILabel()
        label.text = "여기에 유적지 이야기가 표시됩니다."
        label.textAlignment = .center
        label.translatesAutoresizingMaskIntoConstraints = false
        storyVC.view.addSubview(label)
        
        NSLayoutConstraint.activate([
            label.centerXAnchor.constraint(equalTo: storyVC.view.centerXAnchor),
            label.centerYAnchor.constraint(equalTo: storyVC.view.centerYAnchor)
        ])
