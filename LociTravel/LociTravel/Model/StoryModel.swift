...
let dialogues_Temple: [(speaker: String, line: String)] = [
    ("내레이션", "도둑을 쫓아 도착한 곳은 미륵사지였다. 거대한 탑과 돌들이 고요한 침묵 속에 잠들어 있었고, 두 사람은 그 압도적인 분위기에 잠시 말을 잃었다."),
    ("서동", "대체 범인은 어디로 간 거지?"),
    ("내레이션", "그때, 저 멀리서 동자승 차림의 아이가 바쁘게 움직이고 있는 것이 보였다. 아이는 무언가를 찾고 있는 듯했다."),
    ("선화", "저기, 저 아이가…!"),
    ("내레이션", "서동과 선화가 다가가자, 동자승은 겁먹은 표정으로 몸을 웅크렸다. 그의 손에는 서동의 목걸이가 들려 있었다."),
    ("서동", "네가… 내 목걸이를 훔쳐간 범인이니?"),
    ("동자승", "아닙니다, 아닙니다! 저는 그저… 스승님께서 이 목걸이를 가진 사람을 미륵사지로 데려오라 하셔서…!"),
    ("내레이션", "동자승의 말에 서동과 선화는 의아한 표정으로 서로를 바라보았다. 그때, 고요했던 사지에 깊은 목소리가 울려 퍼졌다."),
    ...
    ("내레이션", "고승은 서동의 목걸이를 들고 천천히 읽어내려갔다."),
    ("고승", "王者非血繼之者也... '왕이란 피를 잇지 않은 자가 되려 한다.'"),
    ("서동", "왕이… 피를 잇지 않은 자라고요…? 그게 무슨… 말도 안 되는…."),
    ("내레이션", "얼핏 들으면 반역을 꾀하는 것처럼 들리는 말에 서동은 충격에 휩싸였다. 그의 머릿속은 혼란으로 가득 찼다."),
    ("고승", "선화 공주의 목걸이에 적힌 글은 이것입니다.. 太平之世將至之信... '평화의 시대가 도래할 것이란 우리들의 믿음'"),
    ("내레이션", "두 사람의 목걸이에 담긴 예언은 서동에게는 불안을, 선화에게는 희망을 안겨주었다. 평화의 시대는 과연 어떻게 도래하는 것일까? 그리고 그 시대의 왕은 정말 피를 잇지 않은 자가 되는 것일까? 두 사람의 운명은 미륵사지의 예언과 함께 새로운 국면을 맞이하게 되었다.")
]


// MARK: - 대사 데이터: 서동공원 (Park)
let dialogues_Park: [(speaker: String, line: String)] = [
    ("내레이션", "미륵사지에서 고승의 말을 들은 서동은 깊은 충격과 혼란에 빠졌다. 그는 혹여 자신이 반역을 꾀하는 것으로 오해받을까 두려워, 선화와의 여행을 끝내려 했다."),
    ...
    ("내레이션", "선화의 진심 어린 말에 서동은 흔들리던 마음을 다잡았다. 그때, 서동공원에 갑작스러운 비가 쏟아지며 강물이 불어났다. 강가에 있던 백성들은 순식간에 고립되었고, 백제 왕의 행차마저 발이 묶이는 대혼란이 벌어졌다. 굳건한 왕의 호위병들도 급류 앞에서 속수무책이었다."),
    ("아주머니1", "도와주시오! 아이가 강에 휩쓸려 갔소!"),
    ("내레이션", "서동은 망설이지 않고 급류 속으로 뛰어들었다. 그는 위험을 무릅쓰고 아이를 구했을 뿐만 아니라, 그곳 지리에 밝은 지식을 바탕으로 백성들이 안전하게 대피할 수 있는 길을 찾아냈다. 그의 뛰어난 통찰력과 용기 있는 리더십에 백성들은 물론, 위기에 처했던 백제 왕까지 감탄했다. 왕은 서동에게서 왕의 혈통과는 다른, 백성을 진심으로 위하는 마음과 위기 상황을 해결하는 탁월한 능력을 보았다.")
]
...
let dialogues_Palace_Intro: [(speaker: String, line: String)] = [
    ("내레이션", "백제 왕은 자신과 백성들을 구한 서동에게 깊은 인상을 받았다. 왕에게는 후계자가 없었고, 그는 서동에게서 왕의 자질을 보았다. 백제 왕은 서동에게 양자로 들어와 왕위를 잇고, 백제를 번영시킬 것을 제안했다. 서동은 처음에는 망설였지만, 선화의 응원과 백성들의 환호, 그리고 준위왕의 예언을 떠올리며 자신의 운명을 받아들이기로 결심했다."),
    ("서동", "선화님, 왕궁리 유적으로 갑시다. 이제는 이 땅에 담긴 저의 운명을 마주할 때가 된 것 같습니다."),
    ("내레이션", "두 사람은 함께 왕궁리 유적의 가장 높은 곳에 섰다. 서동은 품속에서 자신의 목걸이 조각을 꺼내 들고, 선화는 자신의 목걸이 조각을 내밀었다. 두 조각이 마침내 하나가 되는 순간, 강력한 빛이 뿜어져 나왔다. 그리고 준위왕의 예언이 허공에 떠올랐다."),
    ("내레이션", "두 사람은 서로의 손을 잡았다. 서동의 눈빛은 흔들림 없는 확신으로 가득 찼고, 선화의 눈빛에는 그를 향한 깊은 신뢰가 담겨 있었다. 그들의 운명적인 만남은 더 이상 우연이 아니었다. 준위왕의 염원이 이 두 사람을 통해 드디어 완성되는 순간이었다.")
